...
namespace ksp2community.ksp2unitytools.editor.Editor.Modding
{
    ...
    public class ModEditor : UnityEditor.Editor
    {
        ...
        {
            var gui = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>("Packages/ksp2community.ksp2unitytools/Assets/Windows/ModEditor.uxml");
            var root = gui.CloneTree();
            ...
            refreshAddressables.clicked += () =>
            {
                (target as Mod)?.CreateAddressablesGroups();
            };
            var refreshThunderkit = root.Q<Button>("RefreshThunderkitButton");
            refreshThunderkit.clicked += () =>
            {
                (target as Mod)?.RefreshPipelines();
            };
            var testInEditor = root.Q<Button>("TestInEditorButton");
            testInEditor.clicked += async () =>
            {
                await KSP2UnityToolsManager.TestModsInPlayMode((Mod)target);
            };
            var testInPlayer = root.Q<Button>("TestInPlayerButton");
            testInPlayer.clicked += async () =>
            {
                await KSP2UnityToolsManager.TestModsInBuiltGame((Mod)target);
            };
            var deployToZipFile = root.Q<Button>("DeployToZipFileButton");
            deployToZipFile.clicked += async () =>
            {
                await KSP2UnityToolsManager.DeployMods((Mod)target);
            };
            return root;
        }
    }
