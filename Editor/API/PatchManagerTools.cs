...
namespace Ksp2UnityTools.Editor.API
{
    ...
    {
        public static readonly List<Type> ModuleDataSubtypes = AppDomain.CurrentDomain
            .GetAssemblies()
            ...
            .Where(type => type
                ...
                .Any(x => x
                    .GetCustomAttributes()
                    .Any(y => y is KSPStateAttribute or KSPDefinitionAttribute)
                )
            )
            .ToList();
        ...
        public static string CreatePatchData(PartData partData, string prefabAddress = "", string iconAddress = "")
        {
            ...
            {
                prefabAddress = $"{partData.partName}.prefab";
            }

            if (string.IsNullOrEmpty(iconAddress))
            {
                iconAddress = $"{partData.partName}.png";
            }
            ...
            foreach (FieldInfo field in partData.GetType()
                .GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
            {
                ...
                {
                    string str = headerAttribute.header;
                    string[] lines = str.Split('\n', StringSplitOptions.RemoveEmptyEntries);
                    int maxLength = lines.Select(x => x.Trim().Length).Max();
                    sb.Append('\n');
                    sb.Indent(indentation);
                    sb.Append("/*\n");
                    sb.Indent(indentation);
                ...
                if (field.Name is "serializedPartModules" or "resourceContainers" or "partName")
                {
                    continue;
                }
                ...
                sb.Append($"{key}: ");
                JToken value = JToken.Parse(IOProvider.ToJson(field.GetValue(partData)));
            ...
            foreach (ContainedResourceDefinition resourceContainer in partData.resourceContainers)
            {
            ...
            sb.Indent(indentation);
            sb.Append("}\n");
            foreach (SerializedPartModule module in partData.serializedPartModules)
            {
        ...
        public static string CreatePatchFileData(string ruleset, object value, params object[] constructorArguments)
        {
            ...
            foreach (object obj in constructorArguments)
            {
                ...
                JToken jsonObject = JToken.Parse(IOProvider.ToJson(obj));
                ...
                .GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
            {
                ...
                sb.Append($"{key}: ");
                JToken val = JToken.Parse(IOProvider.ToJson(field.GetValue(value)));
        ...
        {
            return Regex.IsMatch(name, "[a-zA-Z_]([a-zA-Z_.0-9]|[\\-][a-zA-Z])*");
        }

        public const string DoubleFixedPoint =
            "0.###################################################################################################################################################################################################################################################################################################################################################";

        public static void JsonToDataValue(
            StringBuilder sb,
            JToken token,
            int indentation = -1,
            bool wrapNegatives = false
        )
        {
            if (token == null)
            {
            ...
            while (token.Type == JTokenType.Property)
            {
            ...
            switch (token.Type)
            {
                ...
                {
                    string floatString = ((double)token).ToString(DoubleFixedPoint, CultureInfo.InvariantCulture)
                        .TrimEnd('0');
                    ...
                    if (wrapNegatives && (double)token < 0)
                    {
                ...
                {
                    string intString = ((long)token).ToString(CultureInfo.InvariantCulture);
                    if (wrapNegatives && (long)token < 0)
                    {
                        intString = $"({intString})";
                    }
        ...
        public static void AddDataToModule(StringBuilder sb, object data, int indentation, bool useSemicolons = false)
        {
            ...
            if (data is ModuleData || ModuleDataSubtypes.Contains(objType))
            {
                ...
                {
                    foreach (Data_Engine.EngineMode engineMode in dataEngine.engineModes)
                    {
                ...
                {
                    List<Attribute> attrs = field.GetCustomAttributes().ToList();
                    ...
                    if (field.Name is "engineModes" or "engineID" ||
                        attrs.OfType<JsonIgnoreAttribute>().FirstOrDefault() is not null ||
                        attrs.OfType<KSPStateAttribute>().FirstOrDefault() is not null ||
                        attrs.OfType<KSPDefinitionAttribute>().FirstOrDefault() is null)
                    {
                        continue;
                    }
            ...
            else if (objType.IsArray)
            {
                ...
                {
                    JToken obj = JToken.Parse(IOProvider.ToJson(data));
        ...
        {
            sb.Indent(indentation);
            string name = partModule.Name.Replace("PartComponent", "");
            sb.Append($"+{name} {{\n");
            int nextIndentation = indentation + 1;
            foreach (SerializedModuleData data in partModule.ModuleData)
            {
                sb.Indent(nextIndentation);
                sb.Append($"+{data.Name} {{\n");
                ModuleData obj = data.DataObject;
                int nextNextIndentation = nextIndentation + 1;
