Shader "Redux/Plumes/Volumetric (Profiled)"
{
    Properties
    {
        [Header(General)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float SampleDensity(float3 position){
                ...
                if(length(objectPos.xz) >= .5 - EPSILON)
                return 0;
                ...
                density *= pow(min(max(objectPos.y+.5+(1-_StartFalloffPosition), 0),1), _StartFalloff);
            ...
            fixed4 frag(v2f i) : SV_Target {
                ...
                while(dstTravelled < dstLimit){
                    ...
                    density += SampleDensity(currentPos - position)* stepSize/ sqrt(dot(scale, scale)) * 27;
                    }
